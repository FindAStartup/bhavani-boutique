...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    const isActive = (path) => {
        return location.pathname === path ? 'bg-white/10' : 'hover:bg-white/5';
    };
    ...
    return (
        <>
            ...
            <aside className={`
                fixed top-0 left-0 h-full w-64 bg-primary text-white flex flex-col z-30 transition-transform duration-300 ease-in-out
            ...
            `}>
                <div className="p-8 relative">
                    ...
                    <div className="flex items-center space-x-3 border-b border-white/20 pb-4">
                        <img src={LogoWhite} alt="Bhavana Boutique Logo" className="h-10 w-auto" />
                ...
                <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto custom-scrollbar">
                    {navItems.map((item) => (
                        ...
                        >
                            ...
                            <span className={`font-medium ${location.pathname === item.path ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
                                {item.name}
                            </span>
                        </Link>
                    ))}
